import EncryptedStoraged from 'react-native-encrypted-storage'

const storeLocalData = async (key:string, value:any) =>{
    try{
        await EncryptedStoraged.setItem(
            key,
            value
        )
    } catch(error){
...
const retrieveLocalData =async (key:string) => {
    try{
        const data = await EncryptedStoraged.getItem(key)
...
const removeLocalData =async (key:string) => {
    try{
        await EncryptedStoraged.removeItem(key)
    }catch(error){
